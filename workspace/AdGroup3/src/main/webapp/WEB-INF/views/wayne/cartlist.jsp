				...
				<div id="content">
					...
					<table>
						...
						<c:forEach var="listCart" items="${listCart}">
							<tr>
								<%-- 							<td>${listCart.id}</td> --%>
								<td>${listCart.ad.company}</td>
								<td>${listCart.ad.text }</td>

								<td><a
									href="<c:url value='/cartDelete/${user.id }/${listCart.id}'/>">刪除</a></td>
								<td><input type="button" value="刪除2"
									onclick="deleteCart();" /></td>
							</tr>
						</c:forEach>
