...
window.onload = function() {
	...
	xhr.open("GET", "<c:url value='/orderLists' />", true);
	xhr.send();
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4 && xhr.status == 200) {
			var content = "<table id=myTable><thead>";
			content += "<tr><th>編號</th><th>姓名</th><th>預定時間</th><th>餐廳</th><th>建立時間</th><th>人數</th><th>刪除</th></tr></thead><tbody>";
			var members = JSON.parse(xhr.responseText);
			for(var i=0; i < members.length; i++){
				tmp = "<c:url value='/orderListsEdit/' />";
			    content += 	"<tr><td width='70'>" + members[i].id + "</td>" +
			                "<td align='center'>" + members[i].name + "</td>" +
		        	       	"<td>" + members[i].time + "</td>" +
		            	   	"<td align='right'>" + members[i].resturant + "&nbsp;</td>" +
							"<td>" + members[i].date + "</td>" +
							"<td>" + members[i].people + "</td>" +
							"<td><a href='#'>" + 
